...
pub fn decode_pair_data() -> Vec<Pair> {
    // 反序列化JSON数据到Vec<Pair>
    let json_data = fs::read_to_string("./conf/dex.json")
        .expect("Failed to read dex data");
    let pairs: Vec<Pair> = serde_json::from_str(&json_data).expect("JSON was not well-formatted");
    pairs
}

pub fn construct_pairv2_parameters(pair_info: &Pair) -> Call {
    match pair_info.dex {
        Dex::jedipair => Call {
...
pub fn compile_multicall_parameters() -> Vec<Call> {
    ...
    for (_i, p) in pairs.iter().enumerate() {
